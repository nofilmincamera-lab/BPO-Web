...
async def generate_and_store_embeddings(extraction_result: Dict[str, Any]) -> Dict[str, int]:
    ...
    This task uses GPU acceleration via sentence-transformers while keeping
    entity extraction on CPU (spaCy model).
    """
    logger = get_run_logger()
    ...
    try:
        ...
        logger.warning(f"GPU embeddings module not available: {e}. Skipping embeddings generation.")
        return {"embeddings": 0, "error": "module_not_available"}
    
    # Get embedding info
    info = get_embedding_info()
    logger.info(f"Generating embeddings with model: {info.get('model_name', 'unknown')} on {info.get('device', 'unknown')}")
    
    # Generate entity embeddings (GPU-accelerated)
    try:
...
async def extract_documents_flow(
    ...
    for batch_start, batch_end, batch in _batched_documents(source_path, current_offset, batch_size):
        ...
        # Generate and store GPU-accelerated embeddings
